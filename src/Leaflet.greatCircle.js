...
From https://github.com/nuclearsecrecy/Leaflet.greatCircle
with little adjustements
...
*/
const GreatCircle = L.Circle.extend({
	...
	initialize: function (position, options = {}) {
		...
		this._redraw = function() { this.redraw() };
	},
	...
	_layerAdd: function (e) {
		...
		this._map = map;
		this._map.on("zoomend", this._redraw, this); //refresh on zoom
	...
	redraw: function() {
		...
		if((this._options.radius>=this._options.clipRad || this._clipStatus>1 || l1[0] >= this._options.clipLat || l3[0] <= this._options.clipLat*-1 || this._options.clipLat===false) && this._options.clipLat!==true) {
			...
			var t_start1, t_stop1, t_start2, t_stop2, t_dir1, t_dir2, ll, theta, i, copy;
			switch(this._clipStatus) {
				case 1: //perfect circle -- easy
					t_start1 = 0; t_stop1 = 180;
					t_start2 = 180; t_stop2 = 360;
				break;
				case 2: //top clipping -- work backwards
					t_start1 = 360; t_stop1 = 180;
					t_start2 = 180; t_stop2 = 0;
					this._latLngs.push([90,l2[1]+(360*(-1*this._copies))]);
				break;
				case 3: //bottom clipping -- works best as -180 to 180
					t_start1 = -180; t_stop1 = 0;
					t_start2 = 0; t_stop2 = 180;
			...
			//infer direction
			if(t_start1<t_stop1) { t_dir1 = 1; } else { t_dir1 = -1; }
			if(t_start2<t_stop2) { t_dir2 = 1; } else { t_dir2 = -1; }

			//now we render the circles. we do this for each of the copies.
			for(copy=this._copies*-1;copy<=this._copies;copy++) {
				//iterate over polygon, using geo function to get lat/lng points, for the first half of the circle
				for(theta=t_start1;  t_dir1>0?(theta < t_stop1):(theta > t_stop1);  theta+=(this._options.degStep*t_dir1)) {
					ll = this._destination_from_bearing(lat,lng,theta,this._options.radius*this._m2km);
					ll[1] = ll[1]+(360*copy);
				...
				if(this._clipStatus===4) {
					ll = this._destination_from_bearing(lat,lng,360,this._options.radius*this._m2km);
					this._latLngs.push([ll[0],ll[1]+(360*copy)]);
					if(copy===this._copies*-1) {
						ll = this._destination_from_bearing(lat,lng,-180,this._options.radius*this._m2km);
					...
					if(copy===this._copies) {
						ll = this._destination_from_bearing(lat,lng,0,this._options.radius*this._m2km);
						this._latLngs.push([90,ll[1]+(360*(1*this._copies))]);
					}
				}

				//draw second half of the circle
				for(theta=t_start2;  t_dir2>0?(theta < t_stop2):(theta > t_stop2);  theta+=(this._options.degStep*t_dir2)) {
					ll = this._destination_from_bearing(lat,lng,theta,this._options.radius*this._m2km);
					ll[1] = ll[1]+(360*copy);
					this._latLngs.push(ll);
				}
...
export default function(position, options) {
	return new GreatCircle(position, options);
}
